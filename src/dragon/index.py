# index_template.py
#GENERAL TEMPLATE FOR INDEX STRUCTURE OF RELATIONS 

#BECAUSE WE WİLL DO THE SEARCHING ONLY WITH PRIMARY KEY, WE WİLL USE B+ TREE STRUCTURE FOR INDEXING
...
if __name__ == "__main__":
    ...
    relation_dir = os.path.dirname(__file__) #relation's directory is the same with the index.py's 
    if operation == "create_record":
        key = int(sys.argv[2])
        record = sys.argv[3:]
        success = insert_record(relation_dir, key, record)
        sys.exit(0 if success else 1)
    elif operation == "search_record":
        key = int(sys.argv[2])
        record = search_record(relation_dir, key)
        if record:
            print(record)
            sys.exit(0)
        else:
        ...
        key = int(sys.argv[2])
        success = delete_record(relation_dir, key)
        sys.exit(0 if success else 1)
    else:
        sys.exit(1)
